...
class view
{
    ...
    public function range_jual($tanggal_mulai, $tanggal_akhir) {
        try {
            $sql = "SELECT nota.id_nota, nota.id_barang, nota.jumlah, nota.total, nota.tanggal_input, nota.periode,
                           barang.nama_barang, member.nama_member
                    FROM nota
                    JOIN barang ON nota.id_barang = barang.id_barang
                    JOIN member ON nota.id_member = member.id_member
                    WHERE STR_TO_DATE(nota.tanggal_input, '%Y-%m-%d') BETWEEN :tanggal_mulai AND :tanggal_akhir
                    ORDER BY STR_TO_DATE(nota.tanggal_input, '%Y-%m-%d') ASC";
